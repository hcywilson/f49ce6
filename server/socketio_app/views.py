...
@sio.on("read-message")
def new_message(sid, read_message):
    print(read_message)
    sio.emit(
        "read-message",
        {"conversationId": read_message["conversationId"],
         "readUserId": read_message["readUserId"],
         "lastReadMessageId": read_message["lastReadMessageId"]},
        skip_sid=sid,
    )
